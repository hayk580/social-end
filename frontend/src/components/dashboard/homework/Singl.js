...
export default function HomeworkPage(user, homework) {
    ...
    const [embedUrls,setEmbedUrls] = React.useState();
    ...
    React.useEffect(() => {
        ...
        // axios.get(BACKEND_SERVER_DOMAIN + '/homework/' + slug, config)
    ...
    const commentsToggle = () => {
       ...
       if (comments) {
        ...
        if (comments) {
            ...
            axios.get(BACKEND_SERVER_DOMAIN + "/homeworkcomment/author/" + jsonToken.user.user._id, {headers:{ Authorization: 'Bearer ' +  Token}})
                .then(function (response) {
                    setComments(response.data)
                    if (response.data.comments.length == 0) {
                        setIsLoadingComments(false);
    ...
    if (!userData) {
        ...
        return (!isDeleted) ? (
            <article className="post card">
                 <h6>
                            <Link to={"homework/"+homework._id}>{homework.title} </Link>
                        </h6>
