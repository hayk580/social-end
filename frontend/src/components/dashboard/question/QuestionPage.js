...
import QuestionSingle from './SinglQuestion';
export default function CoursePage() {
    const {course_id} = useParams();
    const user = useSelector((state) => state.user);
    const dispatch = useDispatch();

    const [question,setCourse] = React.useState();
    ...
    React.useEffect(() => {
        ...
        axios.get(BACKEND_SERVER_DOMAIN + '/getschedules/course/'+course_id+"/", config)
            .then(function (response) {
                ...
                console.log("daaa" + JSON.stringify(question))
            })
            .catch(function (err) {
                console.log(err);
                setIsLoading(false)
    ...
    return (
        <section className="profile-page">
            ...
            <QuestionSingle question={question} />
                                ...
                                <div class="sorry">
                                    Add some question and they will show up here!
                                </div>
